...
for (var c = 0; c < 7; c++) {if (window.CP.shouldStopExecution(0)) break;d += '<div class="cell">' + this.DaysOfWeek[c] + "</div>";}window.CP.exitedLoop(0);
d += "</div>";
for (var h, l = dm = "M" == this.f ? 1 : 0 == o ? -5 : 2, v = (c = 0, 0); v < 6; v++) {if (window.CP.shouldStopExecution(1)) break;
d += '<div class="row">';
for (var m = 0; m < 7; m++) {if (window.CP.shouldStopExecution(2)) break;
if ((h = c + dm - o) < 1) d += '<div class="cell disable">' + (u - o + l++) + "</div>";else
if (h > i) d += '<div class="cell disable">' + l++ + "</div>";else
{
...
for (i = 0; i < e.length; i++) {if (window.CP.shouldStopExecution(3)) break;e[i].onclick = function () {
